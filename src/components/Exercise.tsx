...
export default function Exercise({ defaultExerciseName, defaultMax, defaultReps, defaultSets, id }: any) {
    ...
    const handleOnExerciseNameChange = (e: any) => setExerciseName((prev: string) => prev = e.target.value);
    const handleOnMaxChange = (e: any) => setMax((prev: number) => prev = e.target.value);
    const handleOnRepsChange = (e: any) => setReps((prev: number) => prev = e.target.value);
    const handleOnSetsChange = (e: any) => setSets((prev: number) => prev = e.target.value);
    const handleFocusOnExercise = () => setActive(true);
            ...
            <div>
            {
                    active && (
                        <div className="bottom-prompt">
                            ...
                            <button onClick={handleDelete} onTouchEnd={handleDelete} className="btn delete"><FcEmptyTrash size={30} /></button>
                        </div>
                        )
                }
